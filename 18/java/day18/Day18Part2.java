...
public class Day18Part2 {
    ...
    public Day18Part2(Grid grid, int timestamp) {
        this.grid = grid;
    ...
    public static void main(String[] args) throws IOException {
        ...
        new Day18Part2(grid, timestamp).run();
    ...
    private void run() {
        ...
        for (int test = timestamp; test < grid.timestampsCorrupted.size(); ++test) {
            Visit<Vector> path = new AStar().aStar(start, new Day18Strategy(grid, test));
            if (null == path) {
                System.out.println(grid.timestampsCorrupted.get(test));
                return;
            }
            System.out.printf("Timestamp: %d, Path: %d\n", test, path.accumulatedScore);
        }
    }
